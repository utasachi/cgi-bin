...
def search_utanet(utaid):           # 歌ネットタグ取得
    ...
    for retry in range(2):
        ...
        m = re.search(r"<title>(.*?)</title>", texts, re.I | re.S)
    ...
    if tag['title'] == "" or tag['artist'] == "" or tag['year'] == "":
        print(f"タグ情報が不正: utaid={utaid}")
        print(texts)
    return tag

def get_vidids(utaid):           # 歌ネットビデオ情報取得
    time.sleep(1)
    url = f"https://www.uta-net.com/movie/{utaid}/"
    for retry in range(2):
        html = requests.get(url).text
        vidids = list(dict.fromkeys(re.findall(
            r'https://www\.youtube\.com/embed/([^"?/]+)',
            html
        )))
        if vidids: return vidids
        if retry == 0:
            print(f"get_vidids 10秒後に再試行します: utaid={utaid}")
            time.sleep(10)
    print(f"vididが見つかりません: utaid={utaid}")
    print(html)
    return []
...
def get_fname(basedir, tag, ext="mp4",mmtype=""):   # tagからファイルパスを生成
    ...
    if tag.get('tieup'):
        fname = f"{basedir.rstrip('/')}/[{tag.get('tieup')}]{tag['title']}／{artist}{mtype}.{ext}"
    else:
...
def get_ytmusic_duration(videoid):      # ytmusicからduration取得
    try:
        info = ytmusic.get_song(videoid)
        return int(info["videoDetails"]["lengthSeconds"])
    except Exception as e:
        print(f"get_ytmusic_duration エラー: {e}\n{info}")
        return None
...
def write_ass_diag(sinfo,assf,durat=0):                  # ass diag部分記入
